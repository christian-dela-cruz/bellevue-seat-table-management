...
function ModalShell({ children, onClose, disabled, C, maxWidth = 500 }) {
  return (
    ...
    >
      <div style={{ background: C.surfaceBase, borderRadius: 14, width: "100%", maxWidth, maxHeight: "calc(100vh - 40px)", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.35)", border: `1px solid ${C.borderDefault}`, fontFamily: F.body, position: "relative", animation: "modalIn 0.20s cubic-bezier(0.16,1,0.3,1)", overflow: "hidden" }}>
...
function ModalHeader({ eyebrow, title, onClose, disabled, C, meta }) {
  return (
    <div style={{ background: "linear-gradient(160deg,#111009 0%,#1A160F 100%)", padding: "26px 28px 22px", position: "sticky", top: 0, zIndex: 2, borderBottom: `1px solid rgba(255,255,255,0.06)` }}>
...
export default function TwentyTwentyReserveC() {
  ...
  const fetchAndMerge = useCallback(async () => {
    try {
      const res = await fetch(withSeatmapSchedule(`${API_BASE_URL}/reservations?venue_id=3&room=${encodeURIComponent(ROOM)}&per_page=9999`), { headers: { Accept: "application/json" } });
      if (!res.ok) return;
      ...
      setTableData(prev => {
        ...
        if (activeTable) {
          const tables = (prev.tables || []).map(t => {
            ...
            if (mode === "individual") return { ...t, seats: t.seats.map(s => s.id === selectedSeat?.id ? { ...s, status: "available" } : s) };
            let marked = 0;
            return { ...t, seats: t.seats.map(s => { if (marked < guests && s.status === "available") { marked++; return { ...s, status: "available" }; } return s; }) };
          });
  ...
  const canProceed  = (mode === "individual" || currentIsStandalone) && selectedSeat && selectedSeat.status === "available";
  ...
  const rebookPrefill  = rebookFrom ? { firstName: (rebookFrom.name || "").split(/\s+/)[0] || "", lastName: (rebookFrom.name || "").split(/\s+/).slice(1).join(" ") || "", email: rebookFrom.email || "", phone: rebookFrom.phone || "", eventDate: rebookFrom.event_date || "", eventTime: rebookFrom.event_time || "19:00", specialRequests: rebookFrom.special_requests || "", eventArea: rebookFrom.event_area || rebookFrom.eventArea || "", setupTables: rebookFrom.setup_tables || rebookFrom.setupTables || "", setupChairs: rebookFrom.setup_chairs || rebookFrom.setupChairs || "", setupRequirements: rebookFrom.setup_requirements || rebookFrom.setupRequirements || "" } : null;
  const detailsPrefill = formData ? { firstName: formData.firstName || "", lastName: formData.lastName || "", email: formData.email || "", phone: formData.phone || "+63", eventDate: formData.eventDate || "", eventTime: formData.eventTime || "19:00", specialRequests: formData.specialRequests || "", eventArea: formData.eventArea || "", setupTables: formData.setupTables || "", setupChairs: formData.setupChairs || "", setupRequirements: formData.setupRequirements || "" } : (rebookPrefill || { firstName: "", lastName: "", email: "", phone: "+63", eventDate: schedule.eventDate || "", eventTime: schedule.eventTime || "19:00", specialRequests: "" });
