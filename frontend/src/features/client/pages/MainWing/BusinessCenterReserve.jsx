...
function ModalDetails({ tableData, seatData, mode, isStandalone, guests, onReview, onCancel, prefill, C, isDark, secondsLeft, onTimerExpired }) {
  ...
  useEffect(() => {
    if (prefill) setForm({ firstName: prefill.firstName || "", lastName: prefill.lastName || "", email: prefill.email || "", phone: prefill.phone || "+63", eventDate: prefill.eventDate || today, eventTime: prefill.eventTime || "09:00", specialRequests: prefill.specialRequests || "", eventArea: prefill.eventArea || "", setupTables: prefill.setupTables || "", setupChairs: prefill.setupChairs || "", setupRequirements: prefill.setupRequirements || "" });
      ...
      setTableData(prev => {
        ...
        if (isStandalone && snapshotSeat) {
          const updatedStandaloneSeats = (prev.standaloneSeats || []).map(s =>
            s.id === snapshotSeat.id ? { ...s, status: "available" } : s
          );
        ...
        if (activeTable) {
          const tables = (prev.tables || []).map(t => {
            ...
            if (snapshotMode === "individual") {
              return { ...t, seats: t.seats.map(s => s.id === snapshotSeat?.id ? { ...s, status: "available" } : s) };
            }
            let marked = 0;
            return {
              ...t,
              seats: t.seats.map(s => {
                if (marked < snapshotGuests && s.status === "available") { marked++; return { ...s, status: "available" }; }
                return s;
              }),
            };
          });
