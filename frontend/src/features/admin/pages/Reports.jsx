...
import { Building2, Download, Layers, Printer, Utensils, Search, Activity, ChevronDown, CalendarDays, TrendingUp, Users, CheckCircle, Clock, AlertCircle, List, LayoutGrid, X } from "lucide-react";
...
export default function Reports() {
  ...
  const autoGeneratedCsvFilename = useMemo(() => {
    const safeOutlet = String(selectedOutlet || "all")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "");
    if (activeTab === "monthly") {
      return `${safeOutlet}-report-${reportYear}-${String(reportMonth).padStart(2, "0")}-${monthlyGranularity}.csv`;
    }
    if (activeTab === "yearly") {
      return `${safeOutlet}-report-${reportYear}.csv`;
    }
    return `${safeOutlet}-report-${startDate}-to-${endDate}.csv`;
  }, [selectedOutlet, activeTab, reportYear, reportMonth, monthlyGranularity, startDate, endDate]);
  ...
  const filteredTransactionReport = useMemo(() => {
    if (!transactionReport || !transactionReport.data) return { summary: {}, data: [] };
    if (selectedOutlet === "ALL") return transactionReport;
    ...
    const filteredData = transactionReport.data.filter((row) => {
      const room = row.reservation?.room || row.venue?.name || "";
      return childrenSet.has(canonicalOutletName(room));
    });

    let transactions = filteredData.length;
    let approvals = 0;
    let rejections = 0;
    let reverts = 0;
  ...
  }, [selectedOutlet, transactionReport, venueRows]);
  ...
  const transactionSummary = filteredTransactionReport.summary || {};
  const dateRangeLabel = `${readableDate(startDate)} to ${readableDate(endDate)}`;
  const isTrendTab = activeTab === "monthly" || activeTab === "yearly";
  const showOutletFilter = activeTab === "summary" || activeTab === "outlets" || activeTab === "audit";
  ...
  const compileCsvData = () => {
    if (csvExportType === "raw") {
      ...
      // Unified executive multi-table view (3 columns layout)
      const rows = [];
      let sectionCount = 1;
      ...
      if (csvIncludeHeaders) {
        rows.push(["==================================================", "", ""]);
        rows.push([printCustomTitle.trim() || "THE BELLEVUE MANILA", "", ""]);
        rows.push(["Executive Performance & Operational Report", "", ""]);
        rows.push(["Generated At", new Date().toLocaleString(), ""]);
        rows.push(["Date Range", dateRangeLabel, ""]);
        rows.push(["Scoped By", currentUser?.name || currentUser?.email || "Administrator", ""]);
        rows.push(["Format Style", "Unified Executive 3-Column View", ""]);
        rows.push(["==================================================", "", ""]);
        rows.push([], [], []);
      }

      // I. Executive Summary
      if (csvSections.overview) {
        rows.push([`${getSectionNumber()}. Executive Summary`, "", ""]);
        rows.push(["Total Reservations", "", filteredSummary.reservations || 0]);
        rows.push(["Total Guests", "", filteredSummary.guests || 0]);
        rows.push(["Active Outlets", "", selectedOutlet === "ALL" ? (summary.outlets || 0) : filteredOutlets.length]);
        rows.push(["Audit Logs Tracked", "", transactionSummary.transactions || 0]);
        rows.push([], []);
      }

      // II. Financial & Revenue Performance
      if (csvSections.revenue) {
        rows.push([`${getSectionNumber()}. Financial & Revenue Performance`, "", ""]);
        rows.push(["Confirmed Revenue", "", formatCurrency(filteredRevenueSummary.confirmed_revenue)]);
        rows.push(["Projected Revenue", "", formatCurrency(filteredRevenueSummary.projected_revenue)]);
        rows.push(["Avg Guest Spend", "", formatCurrency(filteredRevenueSummary.avg_guest_spend)]);
        rows.push(["Avg Ticket Size", "", formatCurrency(filteredRevenueSummary.avg_ticket_size)]);
        rows.push([], []);
      }

      // III. Distribution & Booking Status Mix
      if (csvSections.mix) {
        rows.push([`${getSectionNumber()}. Distribution & Booking Status Mix`, "", ""]);
        rows.push(["Reservation Status Allocation", "", ""]);
        rows.push(["Reserved / Approved", `${filteredSummary.reserved} bookings`, `${filteredSummary.reservations ? Math.round((filteredSummary.reserved / filteredSummary.reservations) * 100) : 0}%`]);
        rows.push(["Pending Coordination", `${filteredSummary.pending} bookings`, `${filteredSummary.reservations ? Math.round((filteredSummary.pending / filteredSummary.reservations) * 100) : 0}%`]);
        rows.push(["Rejected / Declined", `${filteredSummary.rejected} bookings`, `${filteredSummary.reservations ? Math.round((filteredSummary.rejected / filteredSummary.reservations) * 100) : 0}%`]);
        rows.push(["Cancelled By User", `${filteredSummary.cancelled} bookings`, `${filteredSummary.reservations ? Math.round((filteredSummary.cancelled / filteredSummary.reservations) * 100) : 0}%`]);
        rows.push(["Reservation Category Mix", "", ""]);
        rows.push(["Dine-In Outlets", `${filteredCategory.dine_in?.reservations || 0} bookings`, `${filteredCategory.dine_in?.guests || 0} guests`]);
        rows.push(["Room Service / Tables", `${filteredCategory.room_reservations?.reservations || 0} bookings`, `${filteredCategory.room_reservations?.guests || 0} guests`]);
        rows.push(["Promotions Mentioned", `${filteredCategory.promotion_mentions?.reservations || 0} bookings`, "-"]);
        rows.push([], []);
      }

      // IV. Detailed Venue Performance Listing
      if (csvSections.outlets) {
        rows.push([`${getSectionNumber()}. Detailed Venue Performance Listing`, "", ""]);
        rows.push(["Outlet / Venue Name", "Attribute Description", "Operational Value / Metric"]);
        filteredOutlets.forEach((outlet) => {
          rows.push([outlet.name, "Wing / Type Location", `${outlet.wing || "Main Wing"} (${outlet.type || "Outlet"})`]);
          rows.push(["", "Total Bookings", `${outlet.total_reservations || 0} reservations`]);
          rows.push(["", "Guests Count", `${outlet.guests || 0} guests`]);
          rows.push(["", "Acceptance Rate", `${outlet.acceptance_rate}% (${outlet.reserved || 0} approved, ${outlet.pending || 0} pending)`]);
          if (csvSections.revenue) {
            rows.push(["", "Confirmed Revenue", formatCurrency(outlet.confirmed_revenue)]);
            rows.push(["", "Projected Revenue", formatCurrency(outlet.projected_revenue)]);
            rows.push(["", "Avg Guest Spend", formatCurrency(outlet.avg_guest_spend)]);
          }
          rows.push([], []);
        });
      }

      // V. Room Totals & Location Performance
      if (csvSections.rooms) {
        rows.push([`${getSectionNumber()}. Room Totals & Location Performance`, "", ""]);
        rows.push(["Room / Venue Name", "Attribute Description", "Operational Value"]);
        roomDetails.forEach((room) => {
          rows.push([room.room, "Total Reservations", `${room.reservations || 0} bookings`]);
          rows.push(["", "Total Guests", `${room.guests || 0} guests`]);
          rows.push(["", "Status Breakdown", `Approved: ${room.reserved || 0} | Pending: ${room.pending || 0} | Cancelled: ${room.cancelled || 0}`]);
          rows.push(["", "Dine-In Bookings", `${room.dine_in || 0} bookings`]);
          rows.push(["", "Promo Mentions", `${room.promotion_mentions || 0} promotions`]);
          rows.push(["", "Latest Active Event", room.latest_event_date || "-"]);
          rows.push([], []);
        });
      }

      // VI. Seasonality Trends & Activity Ledger
      if (csvSections.trends) {
        rows.push([`${getSectionNumber()}. Seasonality Trends & Activity Ledger`, "", ""]);
        rows.push(["Annual Month-by-Month Trend", "", ""]);
        (monthlyReport.months || []).forEach((month) => {
          rows.push([month.label, `${month.reservations || 0} reservations`, `${month.promotion_mentions || 0} promo mentions`]);
        });
        rows.push([], []);
        ...
        ((monthlyGranularity === "weekly" ? monthlyReport.selected_month?.weeks : monthlyReport.selected_month?.days) || []).forEach((row) => {
          rows.push([row.label, `${row.reservations || 0} reservations`, `${row.promotion_mentions || 0} promo mentions`]);
        });
        rows.push([], []);
      }

      // VII. Audit Trail Transaction Ledger
      if (csvSections.audit && canViewTransactions) {
        rows.push([`${getSectionNumber()}. Audit Trail Transaction Ledger`, "", ""]);
        rows.push(["Timestamp", "Reference / Guest / Venue", "Action taken / Status Change / Notes"]);
        (filteredTransactionReport.data || []).forEach((row) => {
          const reservation = row.reservation || {};
          rows.push([
            readableDateTime(row.created_at),
            `${reservation.reference_code || "-"} · ${reservation.name || "-"} (${row.venue?.name || reservation.room || "-"})`,
            `${actionLabel(row.action)} (From: ${row.from_status || "-"} To: ${row.to_status || "-"}) - ${row.notes || "-"}`,
          ]);
        });
        rows.push([], []);
      }
        ...
        <div style={{ display: "flex", flexDirection: "column", height: "100vh", flex: 1, minWidth: 0, overflow: "hidden" }}>
          ...
          <main
            style={{
              ...
              overflow: (showPrintConfig || showCsvConfig) ? "hidden" : "auto",
              padding: (showPrintConfig || showCsvConfig) ? 0 : "30px 32px 42px"
            }}
          >
                ...
                actions={canViewReports && !showPrintConfig && (
                  <div className="reports-toolbar print-exclude" style={{ display: "flex", alignItems: "flex-end", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                    {activeTab === "monthly" ? (
                      ...
                      <>
                        {showOutletFilter && (
                          <FilterField label="Outlet / Venue">
                        ...
                        <FilterField label="Preset Period">
            ...
            {!canViewReports ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", background: C.surface, border: "none", borderRadius: 0, overflow: "hidden", fontFamily: F.body, width: "100%", height: "100%" }} className="print-exclude">
                {/* Header */}
                <div style={{ padding: "18px 20px", borderBottom: `1px solid ${C.divider}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, flexShrink: 0 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12, flex: "1 1 auto" }}>
                    ...
                    {/* Date & Outlet filter presets inline right under subtitle */}
                ...
                <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
                  ...
                  <div style={{ width: 320, borderLeft: `1px solid ${C.divider}`, padding: "20px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 20, minWidth: 320 }}>
                    ...
                    <FilterField label="Base Font Size">
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 4 }}>
                        <input
                          ...
                          max="16"
                          step="0.5"
                          value={printFontSize > 16 ? 16 : printFontSize < 8 ? 8 : printFontSize}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          />
                          <span style={{ fontSize: 11, color: C.muted }}>pt</span>
                        </div>
                      </div>
                    </FilterField>
                  </div>
                ...
                <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
                  ...
                  <div style={{ width: 380, borderRight: `1px solid ${C.divider}`, padding: "20px 24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 20, minWidth: 380 }}>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      ...
                      <input
                        type="text"
                        value={csvFilename || autoGeneratedCsvFilename}
                      ...
                      />
                      <div style={{ fontSize: 10.5, color: C.muted }}>Intelligently updates with target outlet and date range filters.</div>
                    </div>
                        ...
                        <div style={{ display: "grid", gap: 10 }}>
                          ...
                          <div style={{ display: "grid", gap: 8 }}>
                            {Object.keys(csvSections).map((sec) => {
                              const isActive = csvSections[sec];
                              return (
                                <div
                                  key={sec}
                                  style={{
                                ...
                                >
                                  <span style={{ textTransform: "capitalize", fontWeight: 600, fontSize: 11.5, color: isActive ? C.text : C.muted }}>
                                    {sec === "overview" ? "Executive Summary"
                                      : sec === "revenue" ? "Revenue & Pricing"
                                        : sec === "mix" ? "Distribution & status mix"
                                          : sec === "trends" ? "Granular trends & charts"
                                            : sec === "audit" ? "Audit Trail Ledger"
                                              : sec}
                                  </span>
                                  <Toggle
                                    checked={isActive}
                                    onChange={(checked) => setCsvSections((prev) => ({ ...prev, [sec]: checked }))}
                                  />
                                </div>
                              );
                            })}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        <span style={{ background: C.goldFaint, color: C.gold, padding: "3px 8px", borderRadius: 99, fontSize: 10, fontWeight: 700 }}>
                          Estimated Columns: {csvExportType === "raw" ? 16 : 3}
                        </span>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, minWidth: 600, color: paperColors.text }}>
                        <thead>
                          <tr style={{ background: paperColors.soft, color: paperColors.faint, borderBottom: `2px solid ${paperColors.gold}` }}>
                            {csvExportType === "raw" ? (
                              ...
                              <>
                                <th style={{ padding: "8px 6px", textAlign: "left", fontSize: 9, fontWeight: 800, letterSpacing: "0.04em", textTransform: "uppercase", width: "30%" }}>Column A</th>
                                <th style={{ padding: "8px 6px", textAlign: "left", fontSize: 9, fontWeight: 800, letterSpacing: "0.04em", textTransform: "uppercase", width: "35%" }}>Column B</th>
                                <th style={{ padding: "8px 6px", textAlign: "left", fontSize: 9, fontWeight: 800, letterSpacing: "0.04em", textTransform: "uppercase", width: "35%" }}>Column C</th>
                              </>
                            )}
                          </tr>
                        </thead>
                        <tbody>
                          {csvExportType === "raw" ? (
                            ...
                            compileCsvData().slice(0, 15).map((row, idx) => (
                              <tr key={idx} style={{ borderBottom: `1px solid ${paperColors.divider}`, background: row[0]?.startsWith("===") ? paperColors.soft : "transparent" }}>
                                <td style={{ padding: "8px 6px", fontWeight: row[0]?.includes(".") || row[0]?.startsWith("===") ? 700 : 400, color: row[0]?.includes(".") ? paperColors.gold : paperColors.text }}>{row[0] || ""}</td>
                                <td style={{ padding: "8px 6px" }}>{row[1] || ""}</td>
                                <td style={{ padding: "8px 6px", fontWeight: typeof row[2] === "number" || (row[2] && (row[2].startsWith("₱") || row[2].endsWith("%"))) ? 700 : 400 }}>{row[2] || ""}</td>
                              </tr>
                            ))
                          )}
                          <tr>
                            <td colSpan={csvExportType === "raw" ? 5 : 3} style={{ padding: "10px 6px", textTransform: "uppercase", fontSize: 9, color: paperColors.faint, fontWeight: 800, textAlign: "center" }}>
                              + {Math.max(0, compileCsvData().length - (csvExportType === "raw" ? 3 : 15))} additional rows will export into spreadsheet document
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  {activeTab === "audit" && canViewTransactions && (
                    <div className="print-audit">
                      <Section title="Audit Trail" subtitle="Read-only status changes for operational review.">
                        <TransactionMonitor transactionReport={filteredTransactionReport} isGlobal={canViewGlobalReports} sort={auditSort} onSort={setAuditSort} />
                      </Section>
                    </div>
                  )}
                </div>
